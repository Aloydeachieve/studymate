...
export default function MaterialDetailPage() {
      ...
      try {
        ...
        try {
          const recRes = await api.get(
            `/study-materials/${id}/recommendations`,
          );
          ...
          if (recRes.data && recRes.data.length > 0) {
            localStorage.setItem(
              "lastRecommendations",
              JSON.stringify(recRes.data.slice(0, 5)),
            );
          }
        } catch (recError) {
  ...
  const handleAiChat = async () => {
    ...
    try {
      ...
      setChatHistory((prev) => [
        ...prev,
        { role: "ai", message: "Error: Could not reach AI service." },
      ]);
    } finally {
      setIsChatting(false);
    }
  ...
  const handleRegenerateSummary = async () => {
    ...
    if (!confirm("Are you sure? This will overwrite the existing summary."))
      return;
    ...
    <div className="min-h-screen bg-gray-50 pb-12">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="py-4 flex items-start sm:items-center justify-between gap-2 sm:gap-4">
            <div className="flex items-start sm:items-center flex-1 min-w-0">
              <button
                onClick={() => router.push("/materials")}
                className="mr-2 sm:mr-4 p-2 text-gray-400 hover:text-gray-600 rounded-full hover:bg-gray-100 transition-colors shrink-0 mt-0 sm:mt-0"
                title="Back to Materials"
              >
                <ChevronLeft size={24} />
              </button>
              <div className="flex-1 min-w-0 pr-2">
                <h1 className="text-lg sm:text-xl font-bold text-gray-900 break-words whitespace-normal leading-snug">
                  {material.title}
                </h1>
                <div className="flex flex-wrap items-center text-xs sm:text-sm text-gray-500 mt-1 gap-y-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {material.content || material.extracted_text || (
                    <div className="text-center py-8 text-gray-500 italic">
                      ...
                      {material.has_extracted_text ? (
                        <span>
                          This document has been processed, but the text preview
                      ...
                      <button
                        onClick={handleDownload}
                        className="mt-4 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded text-gray-800 text-sm font-medium transition-colors"
                      >
                        Download File
                      </button>
                    </div>
                ...
                {analysisResult.summary ? (
                  ...
                  <div className="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-500 text-sm">
                      This document is being processed by our AI. Check back
                      soon.
                    </p>
                    <button
                      onClick={handleRegenerateSummary}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-indigo-50 to-blue-50 rounded-2xl border border-blue-100 p-6 flex flex-col h-[400px]">
              ...
              {/* Input */}
              <div className="flex gap-2 flex-shrink-0 w-full relative">
                <input
                  type="text"
                  value={aiQuery}
                  ...
                  disabled={isChatting}
                  className="flex-1 min-w-0 w-full text-sm border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent p-2 pr-12 disabled:opacity-50"
                />
                <button
                  onClick={handleAiChat}
                  disabled={isChatting}
                  className="shrink-0 bg-blue-600 text-white p-2 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 flex items-center justify-center"
                >
                  {isChatting ? (
                ...
                <div className="mt-6 pt-6 border-t border-gray-100">
                  ...
                  <div className="space-y-2">
                    ...
                    analysisResult.keyTerms.length > 0 ? (
                      analysisResult.keyTerms.map((term: string, i: number) => (
                        <div
                          key={i}
